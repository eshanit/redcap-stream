...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.liver_enrollment_date) {
      try {
        const date = new Date(item.liver_enrollment_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.liver_enrollment_date);
        ...
        console.warn('Invalid date format', item.liver_enrollment_date);
      }
    }
  });
