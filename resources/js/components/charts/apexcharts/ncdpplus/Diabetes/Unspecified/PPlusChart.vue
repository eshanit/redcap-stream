...
interface ActivePatient {
  ...
  db_diagnosis: string | string[] | null;
}
...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Unspecified Diabetes Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.db_visit_date) {
      try {
        const date = new Date(item.db_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.db_visit_date);
        ...
        console.warn('Invalid date format', item.db_visit_date);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.db_visit_date && patient.facility_demo) {
      try {
        const date = new Date(patient.db_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.db_visit_date);
        ...
        console.warn('Invalid date format', patient.db_visit_date);
      }
    }
  });
