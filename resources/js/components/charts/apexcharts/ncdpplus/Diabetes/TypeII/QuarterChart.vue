...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.dm_enrollment_date) {
      const { quarter, year } = getQuarter(item.dm_enrollment_date);
      if (quarter && year) {
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
      <div>
        <h3 class="text-lg font-semibold text-gray-800">Quarterly enrollment trends for Type II Diabetes</h3>
      ...
      <div class="flex flex-wrap gap-3">
        ...
        <div v-if="growthRate && series[0]?.data.length > 1"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td v-for="(count, index) in series[0]?.data" :key="index"
