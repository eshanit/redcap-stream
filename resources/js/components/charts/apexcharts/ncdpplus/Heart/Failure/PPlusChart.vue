...
interface ActivePatient {
  ...
  card_diagnosis: string | string[] | null;
}
...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Heart Failure Disease Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.dateov_visit) {
      try {
        const date = new Date(item.dateov_visit);
        const year = date.getFullYear();
        const quarter = getQuarter(item.dateov_visit);
        ...
        console.warn('Invalid date format', item.dateov_visit);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.dateov_visit && patient.facility_demo) {
      try {
        const date = new Date(patient.dateov_visit);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.dateov_visit);
        ...
        console.warn('Invalid date format', patient.dateov_visit);
      }
    }
  });
...
<template>
  <div>
    ...
    <div v-if="patientsData.length" class="mt-8">
      <h3 class="text-lg font-semibold mb-4">Age Distribution of Active Heart Failure Patients</h3>
