...
interface ActivePatient {
  ...
  liver_outcome: string | null;
  liver_main_diagnosis: string | string[] | null;
}
...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Chronic Liver Disease Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.liver_date) {
      try {
        const date = new Date(item.liver_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.liver_date);
        ...
        console.warn('Invalid date format', item.liver_date);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.liver_date && patient.facility_demo) {
      try {
        const date = new Date(patient.liver_date);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.liver_date);
        ...
        console.warn('Invalid date format', patient.liver_date);
      }
    }
  });
...
<template>
  <div>
    ...
    <div v-if="patientsData.length" class="mt-8">
      <h3 class="text-lg font-semibold mb-4">Age Distribution of Active Chronic Liver Disease Patients</h3>
