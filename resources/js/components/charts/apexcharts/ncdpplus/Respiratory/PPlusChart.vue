...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Chronic Respiratory Disease Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.res_visit_date) {
      try {
        const date = new Date(item.res_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.res_visit_date);
        ...
        console.warn('Invalid date format', item.res_visit_date);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.res_visit_date && patient.facility_demo) {
      try {
        const date = new Date(patient.res_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.res_visit_date);
        ...
        console.warn('Invalid date format', patient.res_visit_date);
      }
    }
  });
...
<template>
  <div>
    ...
    <div v-if="patientsData.length" class="mt-8">
      <h3 class="text-lg font-semibold mb-4">Age Distribution of Active Chronic Respiratory Disease Patients</h3>
