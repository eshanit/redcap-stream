...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.resp_enrollment_date) {
      const { quarter, year } = getQuarter(item.resp_enrollment_date);
      if (quarter && year) {
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
      <div>
        <h3 class="text-lg font-semibold text-gray-800">Quarterly enrollment trends for Chronic Respiratory Disease</h3>
