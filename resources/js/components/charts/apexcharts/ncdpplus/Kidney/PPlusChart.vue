...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Chronic Kidney Disease Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.ckd_visit_date) {
      try {
        const date = new Date(item.ckd_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.ckd_visit_date);
        ...
        console.warn('Invalid date format', item.ckd_visit_date);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.ckd_visit_date && patient.facility_demo) {
      try {
        const date = new Date(patient.ckd_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.ckd_visit_date);
        ...
        console.warn('Invalid date format', patient.ckd_visit_date);
      }
    }
  });
