...
interface ActivePatient {
  ...
  ep_visit_date: string | null;
  ep_next_appo_date: string | null;
  ep_outcome: string | null;
  pat_main_diagnosis: string | string[] | null;
}
...
const chartOptions = ref({
  ...
  title: {
    text: 'Active Epilepsy Patients by Quarter and Facility',
    align: 'center',
    style: {
      fontSize: '16px',
    },
...
const processData = () => {
  ...
  patientsData.value.forEach((item: ActivePatient) => {
    if (item.ep_visit_date) {
      try {
        const date = new Date(item.ep_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.ep_visit_date);
        ...
        console.warn('Invalid date format', item.ep_visit_date);
      }
    }
  });
  ...
  patientsData.value.forEach((patient: ActivePatient) => {
    if (patient.ep_visit_date && patient.facility_demo) {
      try {
        const date = new Date(patient.ep_visit_date);
        const year = date.getFullYear();
        const quarter = getQuarter(patient.ep_visit_date);
        ...
        console.warn('Invalid date format', patient.ep_visit_date);
      }
    }
  });
...
<template>
  <div>
    ...
    <div v-if="patientsData.length" class="mt-8">
      <h3 class="text-lg font-semibold mb-4">Age Distribution of Active Chronic Kidney Disease Patients</h3>
