...
const chartOptions = ref({
  ...
  yaxis: {
    title: {
      text: 'Number of Active Patient(s)',
    },
  },
  fill: {
    opacity: 1,
...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.scd_enrollment_date) {
      try {
        const date = new Date(item.scd_enrollment_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.scd_enrollment_date);
        ...
        console.warn('Invalid date format', item.scd_enrollment_date);
      }
    }
  });
