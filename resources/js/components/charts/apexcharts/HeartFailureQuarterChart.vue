...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.card_enrollment_date) {
      try {
        const date = new Date(item.card_enrollment_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.card_enrollment_date);
        ...
        console.warn('Invalid date format', item.card_enrollment_date);
      }
    }
  });
  ...
  <div class="py-2.5">
     <div class="mt-5 overflow-x-auto">
      <table class="min-w-full bg-white border border-gray-200">
        ...
        <tbody>
          <tr>
            <td class="py-3 px-4 border-b border-gray-200">Number of Patients</td>
