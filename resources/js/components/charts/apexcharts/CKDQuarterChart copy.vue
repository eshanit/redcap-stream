...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.ckd_enrollment_date) {
      try {
        const date = new Date(item.ckd_enrollment_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.ckd_enrollment_date);
        ...
        console.warn('Invalid date format', item.ckd_enrollment_date);
      }
    }
  });
