...
import useInstanceAgeGroups from '@/composables/useInstanceAgeGroups';
import {
    Card,
    CardContent,
    CardDescription,
...
const ageGroups = ['0-10', '11-20', '21-30', '31-40', '41-50', '51-60', '61-70', '70+'];
</script>
<template>
    <Card>
        <CardHeader>
        ...
        <CardContent>
            ...
            <div class="" v-if="props.projectData?.length">
                <!-- <pre>
                    {{ useInstanceAgeGroups(props.projectData) }}
                </pre> -->
                    ...
                    <table class="min-w-full table-auto bg-white shadow-md rounded-lg">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th v-for="label in ageGroups" :key="label" class="px-4 py-2 text-left text-gray-700">
                                    <span v-html="label" /> years old
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(counts, instanceKey) in useInstanceAgeGroups(props.projectData).value"
                                :key="instanceKey" class="hover:bg-gray-50">
                                <td class="border px-4 py-2">{{ getInstanceLabel(instanceKey) }}</td>
                                <td v-for="label in ageGroups" :key="label" class="border px-4 py-2">
