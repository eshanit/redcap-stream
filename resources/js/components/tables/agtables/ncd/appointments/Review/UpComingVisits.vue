...
const transformData = () => {
    const data: any[] = [];
    if (!props.upcomingVisits) return data;

    Object.entries(props.upcomingVisits).forEach(([recordKey, recordData]: [string, any]) => {
        data.push({
            record: recordKey,
            ...recordData
        });
...
watch(() => props.upcomingVisits, (newData) => {
...
const downloadCsv = () => {
    ...
    link.setAttribute('download', `upcoming-visits-${new Date().toISOString().split('T')[0]}.csv`);
...
<template>
    <CustomPaginationTable 
        ...
        description="View scheduled visits that are coming up for patients with expected dates and status."
