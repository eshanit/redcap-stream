...
const transformData = () => {
    const data: any[] = [];
    if (!props.latestVisits) return data;

    Object.entries(props.latestVisits).forEach(([recordKey, recordData]: [string, any]) => {
        data.push({
            record: recordKey,
            ...recordData
        });
...
watch(() => props.latestVisits, (newData) => {
...
const downloadCsv = () => {
    ...
    link.setAttribute('download', `latest-visits-${new Date().toISOString().split('T')[0]}.csv`);
...
<template>
    <CustomPaginationTable 
        ...
        :data="paginationData"
        title="Latest Visits"
        description="View the most recent visits for each patient with status and timing information."
