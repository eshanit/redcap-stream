...
import dataLastThreeMonthsPatients from '@/data/ncdpplus/dataLastThreeMonthsPatients';
// import NoDataSearchIcon from '@/components/custom/NoDataSearchIcon.vue';
import Enrolled from '@/components/tables/agtables/ncdpplus/coreinsights/Enrolled.vue';
...
const type = ref('lastthreemonths')
const tag = ref()



const conditions = [
    { name: 'all', label: 'All Enrolled (Past 3 Months)', tag: 'all' },
...
const selectedComponent = (condition: string) => {
    return dataLastThreeMonthsPatients.find(d => d.name == condition)
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <Heading title="Patients Enrolled Past 3 Months Indicators"
                description="Patients enrolled in the last 3 months per disease/condition aggregated per quarter."
                class="pt-5" />
            ...
            <div v-if="form.condition">
                ...
                <div v-else-if="itemData['all']">

                    <Enrolled :all-results="Object.values(itemData['all'])" />
                </div>
                <div v-else>
                    <div v-if="itemData[form.condition] && itemData[form.condition].length > 0">
                        ...
                        <component :is="selectedComponent(form.condition)?.component"
                            ...
                            :patient-data="itemData[form.condition]" indicator="active" />
                    </div>

                    <!-- No Data Section -->
                    <div v-else
                        class="flex flex-col items-center justify-center border rounded-lg bg-gray-50 h-full min-h-[300px]">
                        <svg class="w-16 h-16 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8v4l3 3m-6 3h6a2 2 0 002-2v-4a2 2 0 00-2-2h-6a2 2 0 00-2 2v4a2 2 0 002 2z" />
                        </svg>
