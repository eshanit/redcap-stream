...
import dataNewLTFUPatients from '@/data/ncdpplus/dataNewLTFUPatients';
...
const type = ref('lastthreemonthsltfu')
const tag = ref()



const conditions = [
    { name: 'all', label: 'All LTFU (Past 3 Months)', tag: 'all' },
...
const selectedComponent = (condition: string) => {
    return dataNewLTFUPatients.find(d => d.name == condition)
}

</script>

<template>
    <Head title="Newly LTFU Dashboard" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                        <Heading title="Patients Newly LTFU Past 3 Months Indicators"
                description="Patients Newly LTFU (In last 3 months) per disease/condition aggregated per quarter." class="pt-5" />
