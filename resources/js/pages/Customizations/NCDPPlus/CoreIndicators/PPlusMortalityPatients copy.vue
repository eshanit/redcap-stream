...
const conditions = [
  { name: 'all', label: 'All Mortality Patients', tag: 'all' },
...
const handleConditionClick = (condition: any) => {
  ...
  router.get(`/packages/project/${props.project.project_id}/core_indicators/nppmortalitypatients/${condition.name}`);
};
</script>
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
      <Heading title="Deceased Patients Indicators"
        description="Registered Deceased Patients per disease/condition aggregated per quarter. Select a condition to view detailed analytics."
        class="pt-5" />
