...
import RheumaticPatientPPlusChart from '@/components/charts/apexcharts/ncdpplus/Heart/Rheumatic/PPlusChart.vue';
...
<template>
  <Head :title="`${project.app_title} - Rheumatic Heart Disease Patients`" />
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <div class="flex items-center gap-4">
              <div class="w-16 h-16 bg-gradient-to-r from-red-500 to-orange-500 rounded-2xl flex items-center justify-center text-3xl">
                ❤️
              </div>
              <div>
                ...
                <p class="text-lg text-slate-600 dark:text-slate-300 mt-3 max-w-3xl">
                  Registered active Rheumatic Heart Disease patients aggregated per quarter and facility.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="text-sm text-slate-500 dark:text-slate-400">Rheumatic Heart Disease Patients</div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-2xl border border-slate-200 dark:border-slate-700 mb-8">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
            <div>
              <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Rheumatic Heart Disease Analysis</h2>
              <p class="text-slate-600 dark:text-slate-300 mt-1">
                Quarterly distribution of active Rheumatic Heart Disease patients across facilities
              </p>
            </div>
            <div class="flex items-center gap-3">
              <div class="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg">
                ...
                <span class="text-sm text-slate-700 dark:text-slate-300">Rheumatic Heart Disease</span>
              </div>
              <div class="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg">
        ...
        <div class="bg-gradient-to-r from-red-50 to-orange-50 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-8 shadow-lg border border-red-100 dark:border-slate-700 mb-8">
          <div class="flex flex-col lg:flex-row items-start lg:items-center gap-6">
            ...
            <div class="flex-grow">
              <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">About Rheumatic Heart Disease</h3>
              <p class="text-slate-600 dark:text-slate-300 mb-4">
                Rheumatic Heart Disease is a serious complication of rheumatic fever, often resulting in permanent damage to heart valves. 
                This dashboard tracks active RHD patients who have had at least one clinical encounter in the last 12 months, 
                with data aggregated quarterly by facility.
              </p>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-2">
                  <div class="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span class="text-sm text-slate-600 dark:text-slate-300">Complication of untreated strep throat infections</span>
                </div>
                <div class="flex items-center gap-2">
                  <div class="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span class="text-sm text-slate-600 dark:text-slate-300">Can cause permanent heart valve damage</span>
                </div>
                <div class="flex items-center gap-2">
                  <div class="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span class="text-sm text-slate-600 dark:text-slate-300">Requires long-term antibiotic prophylaxis</span>
                </div>
                <div class="flex items-center gap-2">
                  <div class="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span class="text-sm text-slate-600 dark:text-slate-300">May require surgical valve repair or replacement</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- Data Debug Section (Optional - can be removed in production) -->
        <div v-if="patients && Object.keys(patients).length > 0" class="mt-8">
          <details class="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
            <summary class="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
              Data Debug Information
            </summary>
            <pre class="mt-2 text-xs text-slate-600 dark:text-slate-400 overflow-auto p-4 bg-slate-100 dark:bg-slate-900 rounded-lg">
{{ JSON.stringify(patients, null, 2) }}
        ...
        <div class="mt-8 text-center text-slate-500 dark:text-slate-400 text-sm">
          <p>© {{ new Date().getFullYear() }} Redcap Stream • Rheumatic Heart Disease Dashboard • {{ project.app_title }}</p>
        </div>
      </div>
    </div>
  </AppLayout>
