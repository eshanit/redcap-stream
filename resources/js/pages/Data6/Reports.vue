...
// "Download PDF" = browser print (Save as PDF as the destination) - same
// mechanism as Overview.vue. The report tables are handled differently from
// Overview's charts though: a chart is one indivisible object so a whole
// chart card gets break-inside-avoid, but a report table can legitimately
// span several pages - only each individual row needs break-inside-avoid,
// so a row's cells never get sliced across a page boundary.
            ...
            <div class="mx-auto max-w-[1500px] px-5 py-7 sm:px-8 lg:px-10 print:max-w-none print:px-0 print:py-0">
                ...
                <template v-else-if="report.length">
                        ...
                        <div class="overflow-x-auto border border-[#d9ded7] bg-[#fcfcfb] print:overflow-visible">
                            <table class="w-full min-w-[860px] text-left text-xs" style="font-variant-numeric: tabular-nums">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-[#eef0eb] text-[#365652] print:break-inside-avoid" :class="expanded.has(row.code) ? 'border-b-0 bg-[#f7f8f4]' : ''">
                    ...
                    <section class="mt-9">
                        ...
                        <p class="mb-2 hidden text-[11px] text-[#a87524] print:block">This table is wide — if columns are cut off at the page edge, switch the print dialog's layout to Landscape.</p>
                        <div class="overflow-x-auto border border-[#d9ded7] bg-[#fcfcfb] print:overflow-visible">
                            <table class="w-full text-left text-xs" style="font-variant-numeric: tabular-nums">
                                <thead class="border-b border-[#d9ded7] bg-[#f0f2ec] text-[10px] font-bold uppercase tracking-wider text-[#5a6f69]">
                                    <tr>
                                        <th class="sticky left-0 bg-[#f0f2ec] px-3 py-2.5 print:static">Indicator</th>
                                        <th v-for="label in gridLabels" :key="label" class="px-3 py-2.5 text-right">{{ label }}</th>
                                        <th class="px-3 py-2.5 text-right">Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="row in report" :key="row.code" class="border-b border-[#eef0eb] text-[#365652] last:border-0 print:break-inside-avoid">
                                        <td class="sticky left-0 max-w-[280px] bg-[#fcfcfb] px-3 py-2 font-semibold text-[#244847] print:static">
