...
import useInstanceHBA1cGroups from '@/composables/useInstanceHBA1cGroups';
import {
    Card,
    CardContent,
    CardDescription,
...
const HBA1cGroups = ['Normal', 'Prediabetic', 'Diabetic'];
    ...
    <Card>
        ...
        <CardContent>
            ...
            <div class="" v-if="props.projectData?.length">
                <!-- <pre>
                    {{ useInstanceHBA1cGroups(props.projectData) }}
                </pre> -->
                    ...
                    <table class="min-w-full table-auto bg-white shadow-md rounded-lg">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th v-for="label in HBA1cGroups" :key="label" class="px-4 py-2 text-left text-gray-700">
                                    <span v-html="label" />
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(counts, instanceKey) in useInstanceHBA1cGroups(props.projectData).value"
                                :key="instanceKey" class="hover:bg-gray-50">
                                <td class="border px-4 py-2">{{ getInstanceLabel(instanceKey) }}</td>
                                <td v-for="label in HBA1cGroups" :key="label" class="border px-4 py-2">
