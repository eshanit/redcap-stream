...
import useInstanceHeightGroups from '@/composables/useInstanceHeightGroups';
import {
    Card,
    CardContent,
    CardDescription,
...
const heightGroups = ['<1', '1-1.49', '1.5-1.99', '2+'];
</script>
<template>
    <Card>
        <CardHeader>
        ...
        <CardContent>
            ...
            <div class="" v-if="props.projectData?.length">
                <!-- <pre>
                    {{ useInstanceHeightGroups(props.projectData) }}
                </pre> -->
                    ...
                    <table class="min-w-full table-auto bg-white shadow-md rounded-lg">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th v-for="label in heightGroups" :key="label" class="px-4 py-2 text-left text-gray-700">
                                    <span v-html="label" /> <span class="italic">m</span>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(counts, instanceKey) in useInstanceHeightGroups(props.projectData).value"
                                :key="instanceKey" class="hover:bg-gray-50">
                                <td class="border px-4 py-2">{{ getInstanceLabel(instanceKey) }}</td>
                                <td v-for="label in heightGroups" :key="label" class="border px-4 py-2">
