...
const processData = () => {
  ...
  props.data.forEach((item) => {
    const enrollmentDate = item.card_enrollment_date || 
                          item.dm_enrollment_date || 
                          item.resp_enrollment_date ||
                          item.ckd_enrollment_date ||
                          item.liver_enrollment_date ||
                          item.scd_enrollment_date;
    ...
    <div class="mt-8 space-y-6">
      ...
      <div>
        <div class="flex items-center justify-between mb-4">
          <h4 class="font-medium text-gray-800">Facility Summary</h4>
          <span class="text-sm text-gray-500">All facilities</span>
        </div>
        
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div 
            v-for="facility in facilityTotals" 
            :key="facility.name"
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="facility in series" :key="facility.name">
                ...
                <td 
                  v-for="(count, index) in facility.data" 
                  :key="index" 
