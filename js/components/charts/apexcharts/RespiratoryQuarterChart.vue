...
const processData = () => {
  ...
  props.data.forEach((item) => {
    if (item.resp_enrollment_date) {
      try {
        const date = new Date(item.resp_enrollment_date);
        const year = date.getFullYear();
        const quarter = getQuarter(item.resp_enrollment_date);
        ...
        console.warn('Invalid date format', item.resp_enrollment_date);
      }
    }
  });
